...
GLboolean rglPutUniformf(GLuint *prog,const char *Name, GLfloat *arg)
{
	...
	switch (count  )
		{	
			...
			break;
			default:
				printf("Error put uniform %s arguments", Name);
				return GL_FALSE;
			break;
		}
	return GL_TRUE;
...
GLboolean rglPutUniformi(GLuint *prog,const char *Name, GLint *arg)
{
	...
	switch (count  )
		{	
			...
			break;
			default:
				printf("Error put uniform %s arguments\n", Name);
				return GL_FALSE;
			break;
		}
	return GL_TRUE;
...
GLboolean rglPutUniformMatrix(GLuint *prog,const char *Name,GLfloat arg[])
{
	GLint count= sizeof(arg)/sizeof(GLfloat);
	switch (count  )
		{	
			...
			break;
			case 4:
				glUniformMatrix2fv(glGetUniformLocation(*prog,Name), count,0,arg);
			break;
			case 9:
				glUniformMatrix3fv(glGetUniformLocation(*prog,Name), count,0,arg);
			break;
			case 16:
				glUniformMatrix4fv(glGetUniformLocation(*prog,Name), count,0,arg);
			break;			
			default:
				printf("Error put uniform %s arguments\n", Name);
				return GL_FALSE;
